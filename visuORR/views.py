...
from .models import VisuORRModel
...
# Vista principal para mostrar los datos de "Visualización Orr"
def visuORR(request):
    ...
    if query:
        try:
            datos = Orr.objects.filter(id=query)
        except ValueError:
            datos = Orr.objects.none()  # Si `query` no es un número válido, no devolvemos nada
    else:
        datos = Orr.objects.all().order_by('-id')

    visualizaciones = VisuORRModel.objects.all()

    # Procesar los datos y calcular porcentajes
    for dato in datos:
        visualizacion = visualizaciones.filter(Orr_id=dato.id).first()
        if visualizacion:
            Preventiva_completa = dato.Accion_Preventiva and dato.Fecha_compromiso2
            if Preventiva_completa:
                dato.paso_4 = True
    ...
    return render(request, "visuORR/visualizacionORR.html", {
        "page_obj": page_obj,
...
def actualizar_checkbox(request):
    if request.method == 'POST':
        ...
        try:
            visualizacion = VisuORRModel.objects.get(Orr_id=Orr_id)
            ...
            if not visualizacion.paso_4 and estado:
                ...
                fecha_inicio = Orr.objects.get(id=Orr_id).fecha_inicio
        ...
        except VisuORRModel.DoesNotExist:
